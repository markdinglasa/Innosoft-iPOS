...
LEFT JOIN MstPayType ON TrnCollectionLine.PayTypeId = MstPayType.Id) ON TrnSales.Id = TrnCollection.SalesId
GROUP BY 
...
IIf([trnSales].[SalesDate]>=[Forms]![RepSales]![DateStart] And [trnSales].[SalesDate]<=[Forms]![RepSales]![DateEnd],True,False), 
TrnSales.IsCancelled,
TrnSales.IsReturn,
TrnSales.IsLocked
HAVING (((Sum(IIf(IsNull([trncollection].[id]),[TrnSales].[Amount],[TrnCollectionLine].[Amount]-IIf([MstPayType].[PayType]="Cash",[TrnCollection].[ChangeAmount],0))))>0) AND ((IIf(IsNull([Forms]![RepSales]![T3_UserIdMem]),"*",Trim(Str([TrnSales].[PreparedBy])))) Like Nz([Forms]![RepSales]![T3_UserIdMem],"*")) AND ((IIf([trnSales].[SalesDate]>=[Forms]![RepSales]![DateStart] And [trnSales].[SalesDate]<=[Forms]![RepSales]![DateEnd],True,False))=True) AND ((TrnSales.IsCancelled)=False) AND (Nz(TrnSales.IsReturn,0)=0)  AND ((TrnSales.IsLocked)=True));
...
GROUP BY IIf(IsNull([trncollection].[Id]),"Uncollected","Collected"), IIf(IsNull([trncollection].[id]),"AR",[mstpaytype].[PayType]), IIf([Forms]![RepSales]![FilterMem]=True,"Filtered",""), IIf(IsNull([Forms]![RepSales]![T3_UserIdMem]),"*",Trim(Str([TrnSales].[PreparedBy]))), IIf([trnSales].[SalesDate]>=[Forms]![RepSales]![DateStart] And [trnSales].[SalesDate]<=[Forms]![RepSales]![DateEnd],True,False), TrnSales.IsCancelled, TrnSales.IsLocked
HAVING (((Sum(IIf(IsNull([trncollection].[id]),[TrnSales].[Amount],[TrnCollectionLine].[Amount]-IIf([MstPayType].[PayType]="Cash",[TrnCollection].[ChangeAmount],0))))>0) AND ((IIf(IsNull([Forms]![RepSales]![T3_UserIdMem]),"*",Trim(Str([TrnSales].[PreparedBy])))) Like Nz([Forms]![RepSales]![T3_UserIdMem],"*")) AND ((IIf([trnSales].[SalesDate]>=[Forms]![RepSales]![DateStart] And [trnSales].[SalesDate]<=[Forms]![RepSales]![DateEnd],True,False))=True) AND ((TrnSales.IsCancelled)=False) AND ((TrnSales.IsLocked)=True));
