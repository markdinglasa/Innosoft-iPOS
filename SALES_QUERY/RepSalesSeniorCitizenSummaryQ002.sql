...
IIf([Forms]![RepSales]![FilterMem]=True,"Filtered","") AS FilterStatus, TrnSales.Pax
FROM (TrnSalesLine INNER JOIN (((((TrnSales 
...
HAVING (((TrnSales.SalesDate) Between [Forms]![RepSales]![DateStart] And [Forms]![RepSales]![DateEnd]) 
...
/*ORIGIN*/
SELECT TrnSales.SalesDate, MstTerminal.Terminal, TrnSales.SalesNumber, IIf([IsCancelled]=0,"Locked","Cancelled") AS Status, TrnSales.SeniorCitizenId, TrnSales.SeniorCitizenName, TrnSales.SeniorCitizenAge, MstUser.username AS [User], MstUser_1.username AS Sales, MstTerm.Term, Sum(IIf([Iscancelled]=True,0,[trnSalesLine].[DiscountAmount]*[trnSalesLine].[Quantity])) AS DiscountAmount, IIf([Iscancelled]=True,0,[trnSales].[Amount]) AS Amount, IIf([Forms]![RepSales]![FilterMem]=True,"Filtered","") AS FilterStatus, TrnSales.Pax
