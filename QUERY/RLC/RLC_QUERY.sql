...
SELECT TrnSales.SalesNumber AS TenantId, 
"00000000000000" & Right([MstTerminal].[Terminal],2) AS TerminalNumber, 
MAX(Nz(RLC_GROSSSALES.[GrossSales],0)) AS GrossAmount, 
0 AS TaxAmount, 
IIf([TrnCollection].[IsCancelled] = 0 OR [TrnCollection].[IsCancelled] IS NULL, 0, FORMAT([GrossSales].[GrossSalesAmount], '0.00000')) AS VoidAmount, IIF([TrnCollection].[IsCancelled] = 0 OR [TrnCollection].[IsCancelled] IS NULL, 0, 1) AS VoidTransaction, 
...
IIf( ([TrnCollection].[IsCancelled] = 0 OR [TrnCollection].[IsCancelled] IS NULL)      AND (TrnSalesLine.DiscountId <> 3 AND TrnSalesLine.DiscountId <> 4)      AND ([TotalDiscount].[TotalDiscountAmount] > 0),      1,      0 ) AS DiscountTransaction, 
...
IIf(     ([TrnCollection].[IsCancelled] = 0 OR [TrnCollection].[IsCancelled] IS NULL)      AND ([MstDiscount].[Discount] = 'Senior Citizen Discount')      AND ([TotalDiscount].[TotalDiscountAmount] > 0),      1,      0 ) AS AdjustmentTransaction, 
...
0 AS PreviousEOD, 0 AS PreviousAmount, 
Nz(DCount("Id","trncollection","[CollectionDate] < " & [Forms]![SysSettings]![RLC_DateMem])+1) AS CurrentEOD, 
MAX(IIF(     [TrnCollection].[IsCancelled] = 0 OR [TrnCollection].[IsCancelled] IS NULL,      FORMAT(Nz([RLC_GROSSSALES].[NetSales], 0), '0.00000'),      '0.00000' )) AS CurrentEODAmount, 
FORMAT([TrnSales].[SalesDate], 'MM/dd/yyyy') AS TransactionDate, 0 AS NoveltyItemAmount, 0 AS MiscItemAmount, IIF([MstTax].[Tax] = 'LOCAL TAX', FORMAT(([TotalTax].[TotalTaxAmount]), '0.00000'), '0.00000') AS LocalTax, 
IIF(([TrnCollection].[IsCancelled] = 0 OR [TrnCollection].[IsCancelled] IS NULL) AND ([TrnCollection].[IsReturn]<>2),
Nz([TmpPayTypeSales].[TotalCreditCardSales],0),0) AS CreditSalesAmount, 0 AS CreditTaxAmount, 
Nz([RLC_GROSSSALES].[VATExempt],0) AS NonVATSalesAmount, 
0 AS PharmaItemSalesAmount, 0 AS NonPharmaItemSalesAmount, 
...
IIF(([TrnCollection].[IsCancelled] = 0 OR [TrnCollection].[IsCancelled] IS NULL) AND      ([TrnCollection].[IsReturn] <> 2) AND      ([TrnSalesLine].[ItemId] = 4),     Nz([TrnSalesLine].[Amount], 0),     0 ) AS GrossSalesAmountNotSubjectToPercentageRent, 
...
LEFT JOIN (SELECT [SalesId], SUM([Amount]) AS GrossSalesAmount FROM TrnSalesLine GROUP BY [SalesId])  AS GrossSales ON [TrnSales].[Id] = GrossSales.[SalesId]) LEFT JOIN (SELECT [SalesId], SUM([Amount]) AS ServiceCharge FROM TrnSalesLine WHERE [ItemId] = 1 GROUP BY [SalesId])  AS TotalServiceCharge ON [TrnSales].[Id] = [TotalServiceCharge].[SalesId]) 
LEFT JOIN (SELECT [SalesId], (SUM([TaxAmount])) AS TotalTaxAmount FROM TrnSalesLine GROUP BY [SalesId])  AS TotalTax ON [TrnSales].[Id] = TotalTax.[SalesId]) LEFT JOIN (SELECT [SalesId], SUM(([DiscountAmount])*([Quantity])) AS TotalDiscountAmount FROM TrnSalesLine GROUP BY [SalesId])  AS TotalDiscount ON [TrnSales].[Id] = [TotalDiscount].[SalesId]
WHERE [TrnSales].[IsLocked] = True      AND [TrnCollection].[IsLocked] = True      AND DAY([TrnSales].[SalesDate]) = DAY([Forms]![SysSettings]![RLC_DateMem])      AND MONTH([TrnSales].[SalesDate]) = MONTH([Forms]![SysSettings]![RLC_DateMem])      AND YEAR([TrnSales].[SalesDate]) = YEAR([Forms]![SysSettings]![RLC_DateMem])
GROUP BY 
DFirst("RLC_TenantId","SysCurrent"), 
"00000000000000" & Right([MstTerminal].[Terminal],2), 
IIf([TrnCollection].[IsCancelled] = 0 OR [TrnCollection].[IsCancelled] IS NULL, 0, FORMAT([GrossSales].[GrossSalesAmount], '0.00000')), IIF([TrnCollection].[IsCancelled] = 0 OR [TrnCollection].[IsCancelled] IS NULL, 0, 1), 
...
IIf(     ([TrnCollection].[IsCancelled] = 0 OR [TrnCollection].[IsCancelled] IS NULL)      AND (TrnSalesLine.DiscountId <> 3 AND TrnSalesLine.DiscountId <> 4)      AND ([TotalDiscount].[TotalDiscountAmount] > 0),      1,      0 ), 
...
IIf(     ([TrnCollection].[IsCancelled] = 0 OR [TrnCollection].[IsCancelled] IS NULL)      AND ([MstDiscount].[Discount] = 'Senior Citizen Discount')      AND ([TotalDiscount].[TotalDiscountAmount] > 0),      1,      0 ), 
Nz(DCount("Id","trncollection","[CollectionDate] < " & [Forms]![SysSettings]![RLC_DateMem])+1), FORMAT([TrnSales].[SalesDate], 'MM/dd/yyyy'), 
IIF([MstTax].[Tax] = 'LOCAL TAX', FORMAT(([TotalTax].[TotalTaxAmount]), '0.00000'), '0.00000'), Nz([RLC_GROSSSALES].[VATExempt],0), 
IIF(     ([TrnCollection].[IsCancelled] = 0 OR [TrnCollection].[IsCancelled] IS NULL) AND      ([TrnCollection].[IsReturn] <> 2) AND      ([TrnSalesLine].[ItemId] = 4),     Nz([TrnSalesLine].[Amount], 0),     0 ), 
