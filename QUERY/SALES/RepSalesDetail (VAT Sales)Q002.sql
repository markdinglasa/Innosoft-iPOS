...
TrnSalesLine.DiscountRate, IIf([Iscancelled]=True,0,[trnSalesLine].[DiscountAmount]) AS DiscountAmount, 
IIf([Iscancelled]=True,0,[trnSalesLine].[NetPrice]) AS NetPrice, TrnSalesLine.Quantity, MstUnit.Unit, 
IIf([Iscancelled]=True,0,[trnSalesLine].[Amount]) AS Amount, 
MstTax.Tax, 
IIf([Iscancelled]=True,0,[trnSalesLine].[TaxAmount]) AS TaxAmount, 
...
AND ((IIf([Iscancelled]=True,0,[trnSalesLine].[TaxAmount]))>0) 
...
SELECT TrnSales.SalesDate, TrnSales.SalesNumber, MstCustomer.Customer, TrnSales.Remarks, MstUser_1.UserName AS [User], MstUser.UserName AS Sales, MstItem.ItemDescription, TrnSalesLine.Preparation, MstItem.Category, IIf([Iscancelled]=True,0,[trnSalesLine].[Price]) AS Price, TrnSalesLine.DiscountRate, IIf([Iscancelled]=True,0,[trnSalesLine].[DiscountAmount]) AS DiscountAmount, IIf([Iscancelled]=True,0,[trnSalesLine].[NetPrice]) AS NetPrice, TrnSalesLine.Quantity, MstUnit.Unit, IIf([Iscancelled]=True,0,[trnSalesLine].[Amount]) AS Amount, MstTax.Tax, IIf([Iscancelled]=True,0,[trnSalesLine].[TaxAmount]) AS TaxAmount, IIf([Forms]![RepSales]![FilterMem]=True,"Filtered","") AS FilterStatus
